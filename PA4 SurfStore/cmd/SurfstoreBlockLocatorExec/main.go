...
func main() {
	...
	if *downServers != "" {
		for _, downServer := range strings.Split(*downServers, ",") {
			intDown, err := strconv.Atoi(downServer)
			if err != nil {
				log.Fatal("downserver int convert err", err)
			}
			downList = append(downList, intDown)
	...
	c := surfstore.NewConsistentHashRing(numServers, downList)
	...
	for {
		...
		buf = buf[:l]
		hashString := surfstore.GetBlockHashString(buf)
		blockHashList = append(blockHashList, hashString)
	}

	mapping := c.OutputMap(blockHashList)
	ans := "{"
	for i := 0; i < len(blockHashList); i += 1 {
		currBlock := blockHashList[i]
		idx := "block" + strconv.Itoa(i)
		serverForBlock := mapping[idx]
		idx2 := serverForBlock[len(serverForBlock)-1:]
		ans += "{" + currBlock + "," + idx2 + "},"
	}
